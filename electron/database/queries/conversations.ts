import { getDatabase, withSchemaRetry } from '../index'
...
export function addMessage(
  ...
  return withSchemaRetry(() => {
    ...
    // Wrap INSERT + UPDATE in a transaction for atomicity and performance
...
export function getConversationsByDocument(documentId: string): ConversationSummary[] {
  return withSchemaRetry(() => {
    const db = getDatabase()
    // Use subquery with ROW_NUMBER to get last message more efficiently than correlated subquery
    return db.prepare(`
